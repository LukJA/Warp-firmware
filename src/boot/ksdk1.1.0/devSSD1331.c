...
// Bitmap Index,  W, H, xAdv, dX, dY
// const int org_glyphdata[95][6] = {{0, 0, 0, 6, 0, 1},     // 0x20 ' '
//                         {0, 1, 5, 2, 0, -4},    // 0x21 '!'
//                         {1, 3, 1, 4, 0, -4},    // 0x22 '"'
//                         {2, 5, 5, 6, 0, -4},    // 0x23 '#'
//                         {6, 5, 5, 6, 0, -4},    // 0x24 '$'
...
static int writeData(uint16_t dataByte)
{
	spi_status_t status;
	/*
	 *	Drive /CS low.
	...
	GPIO_DRV_ClearPinOutput(kSSD1331PinCSn);
	/*
	 *	Drive DC High (data mode).
	 */
	GPIO_DRV_SetPinOutput(kSSD1331PinDC);

	payloadBytes[0] = dataByte & 0xFF;
	payloadBytes[1] = (dataByte >> 8) & 0xFF;
	status = SPI_DRV_MasterTransferBlocking(0	/* master instance */,
					NULL		/* spi_master_user_config_t */,
					(const uint8_t * restrict)&payloadBytes[0],
					(uint8_t * restrict)&inBuffer[0],
...
// // copy the frame buffer into the display memory
// // monochromatic
// int devSSD1331_displayFrameBuffer(void){
// 	// set col and row pointers to 0
// 	// col is auto increment, row does not
...
// 		writeCommand(y);
// 		for (uint8_t x = 0; x < 94; x = x + 8){
// 			uint8_t byte = frameBuffer[y][x];
...
// 	writeCommand(kSSD1331CommandCLEAR);
// 	writeCommand(0x00); // start x
// 	writeCommand(0x00); // start y
// 	writeCommand(0x5F); // finish x = 95
// 	writeCommand(0x3F); // finish 7 = 63
// 	return 0;
// }
